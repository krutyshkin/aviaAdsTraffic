...
// Aviamasters UI Module — всё изолировано
(() => {
// ==UserScript==
...
(() => {
  ...
  CanvasRenderingContext2D.prototype.fillText = function (text, x, y, ...args) {
    ...
    for (const [key, val] of Object.entries(languages.en)) {
        const __translation_ui_safe__ = languages[fallbackLang][key];
    
          const __translation_ui_safe__ = languages[window.currentLang][key];
          const regex = new RegExp(val, 'gi');
          text = text.replace(regex, __translation_ui_safe__);
        }
