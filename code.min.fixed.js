...
(function() {
  var validCodes = ['NVo2MS1VQ1RP', 'OTdLNy1VNlBW', 'NkNVNi1WQk5D', 'TjM2NC1BQ0pJ', 'TjlKVi1HV05P', 'QVVSTC1FVU9J', 'VVpUWC1UREZD', 'NTJROS1FVU5G', 'UEFLMC1RWlNP', 'Q0VXNS1QMDFF', 'Q1kxVy1ZTkQw', 'UkVETi1UUlFI', 'ME1LUi1PMUxN', 'OVJGMS1aVTNY', 'VEU4UC1BUEpZ'];
  var now = Date.now();
  var saved = JSON.parse(localStorage.getItem("aviacode_cloud") || "null");

  function activate() {
    document.addEventListener("DOMContentLoaded", function() {
      console.log("✅ DOM загружен, активируем UI");
      start();
    });
  }
  ...
  function start() {
(() => {
  var baseBet = parseFloat(localStorage.getItem('baseBet')) || 100000;
  var currency = localStorage.getItem('currency') || 'IRT';
  window.currentLang = localStorage.getItem('currentLang') || 'en';

  var currencies = ['RUB', 'USD', 'VND', 'IRT', 'ARS', 'EUR', 'IDR', 'MDL', 'KWD', 'BHD', 'OMR', 'BTC'];
  var languages = {
  ...
  CanvasRenderingContext2D.prototype.fillText = function (text, x, y, ...args) {
    try {
      if (typeof text === 'string') {
        ...
        var isSpecialSymbol = /^[+×/÷-]\s?\d{1,3}$/i.test(text.trim());
        if (isSpecialSymbol) {
          return originalFillText.call(this, text, x, y, ...args);
        }

        // Проверяем позицию для множителя (обычно справа внизу)
        var isMultiplierArea = x > 500 && y > 400; // Примерная область множителя
        ...
        for (var [key, val] of Object.entries(languages.en)) {
          var translation = languages[window.currentLang][key];
        ...
        if (!isNaN(num)) {
          var currentBaseBet = parseFloat(localStorage.getItem('baseBet')) || 100000;
          var multiplier = num / 1000;
          var value = currentBaseBet * multiplier;
        ...
        while (this.measureText(text).width > maxWidth) {
          var currentSize = parseInt(this.font.match(/\d+/)[0]);
  ...
  var createSettingsButton = () => {
    ...
    btn.addEventListener('click', () => {
      ...
      document.getElementById('apply-settings').onclick = () => {
        var newCurrency = document.getElementById('currency-input').value;
        var newBet = parseFloat(document.getElementById('bet-input').value);
